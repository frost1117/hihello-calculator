...
export const useCalculator = (): CalculatorHookResult => {
  ...
  const newDisplayState = {
    display,
    clearText: canClearLastEntry(internalState)
      ? clearText.clear
      : clearText.allClear,
  };
  ...
  const actions = {
    appendNumber: (value: string) =>
      update(calculatorActions.appendNumber(internalState, value)),
    setOperator: (operator: string) =>
      updateWithDisplayFlash(
        calculatorActions.setOperator(internalState, operator)
      ),
    calculateTotal: () =>
      updateWithDisplayFlash(calculatorActions.calculateTotal(internalState)),
    clear: () => updateWithDisplayFlash(calculatorActions.clear(internalState)),
    appendDecimalPoint: () =>
      update(calculatorActions.appendDecimalPoint(internalState)),
    toggleSign: () =>
      updateWithDisplayFlash(calculatorActions.toggleSign(internalState)),
    applyPercent: () =>
      updateWithDisplayFlash(calculatorActions.applyPercent(internalState)),
  };
